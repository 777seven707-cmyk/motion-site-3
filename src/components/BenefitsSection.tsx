import { ShieldCheck, Clock, Unlock, TrendingUp, ShoppingBag, BarChart3 } from 'lucide-react';
      ...
      <div className="max-w-[1280px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-white/10 mt-[56px] md:mt-[72px]">
          {benefits.map((b, i) => {
            const Icon = b.icon;
            return (
              <Reveal key={b.num} delay={(i % 3) * 90} className="bg-[#050505] group relative overflow-hidden">
                ...
                <span
                  aria-hidden="true"
                  className="pointer-events-none absolute -top-[18px] right-[8px] font-graphik font-bold text-white/[0.05] text-[110px] leading-none select-none transition-colors duration-500 group-hover:text-white/[0.08]"
                >
                  {b.num}
                </span>
                <div className="relative h-full p-[28px] md:p-[34px] transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:bg-white/[0.025] group-hover:-translate-y-[4px]">
                  <span
                    className="relative flex items-center justify-center w-[46px] h-[46px] rounded-full border border-white/20 transition-all duration-500 group-hover:border-white/50"
                    style={{ boxShadow: '0 0 0px rgba(255,255,255,0)' }}
                  >
                    <span className="absolute inset-0 rounded-full bg-white/0 blur-md transition-all duration-500 group-hover:bg-white/20" />
                    <Icon className="relative w-[20px] h-[20px] text-white" strokeWidth={1.5} />
                  </span>
                  <h3 className="font-graphik text-white text-[20px] md:text-[23px] font-normal leading-[1.2] mt-[20px] mb-[14px]">
                    {b.title}
                  </h3>
